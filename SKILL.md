---
name: v8std-standards-harness
description: >
  Use when writing, reviewing, or designing 1C:Enterprise (BSL) code and configuration
  metadata — enforces a fail-closed check against official 1C development standards (v8std)
  instead of relying on the agent to remember them. Triggers on: writing a query, creating
  or changing a metadata object (catalog, document, register, form, role), document posting
  and register movements, export procedures, client-server transitions, privileged mode,
  localization, configuration extensions, and any BSL code review. Produces machine-readable
  evidence lines that a validator can gate on, so a skipped standards check cannot pass
  silently. Not for non-1C codebases.
---

# Обвязка для работы со стандартами 1С (v8std)

Обвязка вокруг MCP-сервиса стандартов разработки 1С (`https://ai.v8std.ru/mcp`).

Решает одну конкретную задачу: **не дать агенту молча пропустить сверку со стандартами**. Он не
врёт и не ошибается — он просто не считает проверку обязательной, а вы узнаёте об этом на
проверке кода или в работающей базе. Обычная инструкция «сверяйся со стандартами» эту проблему не решает,
потому что её невыполнение ничем не отличается от её выполнения.

## Как это устроено: четыре слоя

Ни один слой не самодостаточен. Работают только вместе.

| Слой | Что ловит | Что протекает без него |
|---|---|---|
| **1. Карта ситуаций** | Известные случаи: «пишешь запрос → std729/437/438» | Агент сверяется по настроению и по памяти |
| **2. Обязательный поиск** | Стандарты вне карты — минимум один `v8std_search` за этап | Карта замерзает в моменте создания и тихо устаревает |
| **3. Сторожевая проверка** | Устаревший или недоступный указатель стандартов | «Ничего не нашлось» неотличимо от «сервис лежит» |
| **4. Проверка отметок** | Саму молчаливую деградацию — отсутствие следа проверки | Все три слоя выше — необязательная рекомендация |

Карта — в двух проекциях:
- [`references/standards-map.md`](references/standards-map.md) — по объекту метаданных:
  «создаёшь регистр → …», «настраиваешь RLS → …»;
- [`references/situations.md`](references/situations.md) — по действию в коде:
  «пишешь `ОбработкаПроведения` → …», «составной тип в запросе → …».

Почему именно четыре слоя и какие случаи из практики их породили —
[`references/field-notes.md`](references/field-notes.md).
Как поставить это у себя — [`references/INSTALL.md`](references/INSTALL.md).

## Слой 1: карта ситуаций

1. Определи ситуацию: что ты делаешь с кодом и с какими объектами метаданных.
2. Найди её в одной из двух проекций карты.
3. Прочитай указанные стандарты через MCP: `v8std_get_page("std657")` — **строкой**, не
   числом (`v8std_get_page(657)` вернёт `found=false`).
4. Примени и оставь запись (см. «Формат отметок»).

Уровни: `always` — сверка обязательна всегда при срабатывании условия; `conditional` —
при наличии подситуации; `optional` — для нестандартных решений.

## Слой 2: обязательный поиск

Карта — короткий список, а не исчерпывающий перечень. Поэтому **минимум один `v8std_search` за
этап работы**, даже когда карта, кажется, всё покрыла.

1. Возьми `search_queries` своей ситуации из
   [`references/situations.md`](references/situations.md) (раздел «Признаки ситуаций и запросы поиска»)
   либо сформулируй свой запрос и обоснуй его в записи.
2. Разбери результаты. Номер, которого нет в карте, проверь через `v8std_get_page(<номер>)`.
3. Реши: `applied` (применил) / `noted` (учёл на будущее) / `not_relevant` (не подходит).
4. Оставь запись `[v8std discovered: ...]`.

Если ни одна ситуация не сработала — **всё равно** один поиск по теме работы плюс запись
`[v8std skipped: ..., reason=no_matching_situation]`. Молчаливый пропуск запрещён: именно он
и есть тот отказ, против которого построена вся обвязка.

## Слой 3: сторожевая проверка

Один раз за задачу запроси заведомо существующий номер стандарта: `v8std_get_page("<номер из sentinelId>")`.

- Ожидание — `found=true`, запись `[v8std sentinel: id=<...>, status=found, phase=<...>]`.
- `found=false` или MCP не отвечает после 3 попыток → `[v8std skipped: ...,
  reason=stale_or_unavailable_index]`.

**Эталонный номер фиксируется при установке** — возьмите самый свежий стандарт, который сегодня
отдаёт MCP, и пропишите его в `sentinelId` настроек. Незафиксированный эталон бесполезен.

> **Честная граница слоя.** Она ловит недоступность сервиса, исчезновение номера и откат
> указателя назад. Он **не ловит противоположное** — что индекс ушёл вперёд, а ваша карта
> осталась в прошлом. Против этого работает только слой 2 плюс датированная ревизия карты
> (`last_reviewed_at`).

## Слой 4: формат отметок и их проверка

Каждая проверка оставляет **одну строку** в секции `## v8std evidence` вашего рабочего
артефакта (по умолчанию — `v8std-evidence.md` в корне ветки; настраивается).

```
[v8std applied: phase=<этап>, scope=<ситуация>, ids_checked=[stdA,stdB], conclusion=clean|violation:<id>]
[v8std skipped: phase=<этап>, scope=<ситуация>, planned_ids=[stdA,stdB], reason=<причина>, retries=3]
[v8std discovered: phase=<этап>, scope=<ситуация>, query="<запрос>", top_ids=[stdA,stdB], new_ids=[stdB], decision=applied|noted|not_relevant]
[v8std sentinel: id=<stdNNN>, status=found|not_found, phase=<этап>]
```

| Поле | Допустимые значения |
|------|---------------------|
| `phase` | Любая непустая метка вашего процесса. Если задан `phases` в настройках — сверяется по нему |
| `scope` | Короткий kebab-case: `document-posting`, `new-common-module`, `query-virtual-table`, `no-matching-situation` |
| ID-поля | `stdNNN` (`std450`), `acc:NNN` (`acc:105`), `bslls:Code` (`bslls:MissingSpace`), `v8cs:code` (`v8cs:module-region-empty`), `patterns:alias` и подстраницы (`patterns:solid:single_responsibility`). Списки не могут быть пустыми — кроме `new_ids`, где пустота законна |
| `reason` | `mcp_unavailable_after_3_retries`, `timeout`, `auth_error`, `parse_error`, `tool_not_found`, `no_matching_situation`, `stale_or_unavailable_index` |
| `decision` | `applied`, `noted`, `not_relevant` |
| `conclusion` | `clean` или `violation:<id>` |
| `status` | `found`, `not_found` |

**Повторные попытки:** при недоступности MCP — 3 попытки с задержками 1 с / 3 с / 9 с. Только
после третьей оформляется `skipped`. Меньше трёх — нарушение, программа проверки выдаст предупреждение.

Запуск проверки:

```
node tools/v8std-evidence-validator.mjs validate-pack <каталог>          # режим из настроек
node tools/v8std-evidence-validator.mjs validate-pack <каталог> --gate   # строгий режим
node tools/v8std-evidence-validator.mjs validate-pack <каталог> --lint   # только оформление
```

| Профиль | Поведение |
|---|---|
| `lint` | Проверяет только оформление найденного. Ноль записей = чисто. Для ручного прогона |
| `gate` | Запрет по умолчанию. Требует: заданный `sentinelId` в настройках; минимум одну запись; сторожевую запись с этим `id` и `status=found`; поиск или обоснованный пропуск. **Ноль записей = код возврата 2.** Для сборки и проверки перед коммитом |

Оба режима отвергают записи, которые лишь выглядят заполненными:

- пустые значения обязательных полей (`id=`, `status=`, `query=`) — включая пробелы
  и невидимые символы вроде `U+200B`;
- пустые обязательные списки (`ids_checked=[]`, `top_ids=[]`, `planned_ids=[]`) —
  «проверил ничего» не является проверкой; исключение — `new_ids`, где пустота законна;
- подмену типа: `ids_checked="[]"` (строка вместо списка), `ids_checked=NOT_AN_ID`
  (скаляр вместо списка), `query=[]` (список вместо скаляра).

**Настройки задаёт проект, а не проверяемая задача.** Если файл настроек есть и в корне
проекта, и в каталоге задачи, строгость (`profile`, `sentinelId`, `phases`) берётся из
проектного; локальный может уточнять только раскладку файлов. Иначе достаточно положить
рядом с задачей свой файл настроек, чтобы назначить себе эталон и пройти проверку.

**`sentinelId` может быть списком.** Первый элемент — актуальный, остальные допустимы для
уже завершённых работ. Так эталон поднимается, не объявляя нарушителями старые задачи;
в строгом режиме прежнее значение даёт предупреждение.

Строгий режим `gate` дополнительно блокирует:

- `sentinel` со `status=not_found` — если источник истины протух, результаты всей сессии
  недостоверны;
- нераспознанный идентификатор в списках (в `lint` это остаётся предупреждением);
- повтор ключа в записи (`status=not_found, status=found`) — вторым значением легко
  переопределить проверку;
- запись, оформленную не одной строкой, и любую строку с маркером `[v8std`, которую
  не удалось разобрать: невидимое нарушение опаснее явного;
- отметку «подходящих ситуаций не нашлось» (`reason=no_matching_situation`) без записи
  `discovered`: «не нашлось» — это вывод поиска, а не право не искать.

Записи не засчитываются, если они выключены разметкой: внутри HTML-комментария (где бы он
ни находился), внутри кода-примера в документации (заголовок секции сам стоит в код-блоке)
или за пределами секции. Заголовок секции распознаётся на уровнях 2 и 3 — в отчёте задачи
она часто вложена в раздел; закрывает секцию заголовок того же или более высокого уровня.

Коды выхода: `0` — чисто, `1` — только предупреждения, `2` — блокирующие нарушения.

## Что этот навык не делает

- **Не анализирует код.** v8std — справочник стандартов и диагностик. Он не заменяет
  синтаксическую проверку, статический анализатор и прогон тестов.
- **Не гарантирует, что стандарт применён правильно.** Валидатор проверяет наличие и
  формат следа, а не качество решения. Это защита от пропуска, а не от ошибки.
- **Не обновляет карту сам.** Карта — снимок на дату. Пересмотр — ваша ответственность,
  рецепт в [`references/INSTALL.md`](references/INSTALL.md).
